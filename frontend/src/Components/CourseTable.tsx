    ...
    <div className="overflow-x-auto">
      ...
      <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          ...
          {courses.map((CourseWithMetaData, index) => (
            // The css means to create alternating row colours, odd rows are white and even rows are light gray.
            <tr key={CourseWithMetaData.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                {CourseWithMetaData.course_code}
              </td>
              {/* Third column for copyright information */}
              <td className="px-6 py-4 text-sm text-gray-500">
                <div className="space-y-1">
                  <div>
                    <span className="font-medium">Owner:</span> {CourseWithMetaData.copyrightOwner.name} ({CourseWithMetaData.copyrightOwner.type})
                  </div>
                  <div>
                    <span className="font-medium">Licence:</span> {CourseWithMetaData.copyrightOwner.license_type}
                    {CourseWithMetaData.copyrightOwner.license_url && (
                      <a
                        href={CourseWithMetaData.copyrightOwner.license_url}
                        target="_blank"
                        ...
                        aria-label={`View licence details for ${CourseWithMetaData.title}`}
                      >
                        Details
                      </a>
                    )}
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                <div>
                  <h4 className="font-medium">{CourseWithMetaData.topic.name}</h4>
                  <p className="text-sm text-gray-600 mt-1">{CourseWithMetaData.topic.name}</p>
                </div>
              </td>
              {/* Fifth column for enrol action */}
              <td className="px-6 py-4 whitespace-nowrap">
                <button
                  onClick={() => enrol(CourseWithMetaData.id)}
                  disabled={enrolStatus[CourseWithMetaData.id] === "Processing..."}
                  ...
                  aria-label={`Enrol in ${CourseWithMetaData.title}`}
                >
                  {enrolStatus[CourseWithMetaData.id] === "Enrolled!" ? "Enrolled" : "Enrol"}
                </button>
                {/* Display enrolment status if CourseWithMetaData.id is true and enrolStatus is not "Enrolled!" */}
                {enrolStatus[CourseWithMetaData.id] && enrolStatus[CourseWithMetaData.id] !== "Enrolled!" && ( 
                  <span>{enrolStatus[CourseWithMetaData.id]}</span> //This will only show if above conditions are true
                )}
              </td>
            </tr>
          ))}
