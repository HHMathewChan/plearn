                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {enrolledCourses.map((course, index) => (
                        <tr key={course.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            ...
                            {/* Difficulty - Placeholder logic based on course code */}
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                                    course.course_code.includes('1') ? 'bg-green-100 text-green-800' :
                                    course.course_code.includes('2') ? 'bg-yellow-100 text-yellow-800' :
                                    'bg-red-100 text-red-800'
                                }`}>
                                    {course.course_code.includes('1') ? 'Beginner' :
                                     course.course_code.includes('2') ? 'Intermediate' : 'Advanced'}
                                </span>
                            </td>
