 ...
 * and return the student_code, platform_user_id and JWT token if the login is successful
 * @param {string} email
 *   Email address of the user.
 *   - Domain: EmailAddress (must be unique, valid email format)
 * @param {string} password
 *   Password of the user.
 *   - Domain: Password (supposed to be hashed but now just a string)
 * @returns {Object}
 *   An object containing the student_code, platform_user_id, and JWT token.
 *   - student_code: Unique code for the student, generated by the system.
 *   - platform_user_id: Unique identifier for the platform user.
 *   - token: JWT token for the student.
 */
const verifyStudentLogin = async (email, password) => {
  ...
  if (password_hash === password) {
    ...
    // gnerate a JWT token for the student
    const token = generateToken({platform_user_id, student_code});
    // console.log("token", token);
    return {student_code, platform_user_id, token};
  } else {
